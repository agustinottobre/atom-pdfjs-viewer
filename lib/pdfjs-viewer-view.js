...
export default class PdfjsViewerView {
  ...
  constructor(pathname) {
    ...
    this.element.src = viewerPathname + '?file=' + pathname + '#pagemode=none&zoom=page-fit'
  ...
  reloadPdf() {
    ...
    if (win) {
      ...
      const overrideFingerprint =
        atom.config.get('pdfjs-viewer.overrideFingerprint')
      if (!overrideFingerprint) {
        win.PDFViewerApplication.close()
        win.PDFViewerApplication.open(this.pdfPathname)
      } else {
        // Using the trick to override the PDF ID from
        // https://github.com/mozilla/pdf.js/issues/11359#issuecomment-659154737
        win.pdfjsLib.getDocument(this.pdfPathname).promise
          .then(doc => {
            console.log('PdfjsViewerView: overriding fingerprint')
